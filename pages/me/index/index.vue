<template>
	<view>
		<view class="header">
			...
			<view class="setting" @tap="out"><image src="../../../static/HM-PersonalCenter/setting.png"></image></view>
		</view>
		<view class="orders">
			<view class="box">
				<view class="label" v-for="(row,index) in orderTypeLise" :key="row.name" hover-class="hover"  @tap="toOrderType(index)">
					<view class="icon"><view class="badge" v-if="row.badge>0">{{row.badge}}</view><image :src="row.icon"></image></view>
					{{row.name}}
				</view>
			</view>
		</view> 
		...
		data() {
			return {
				...
				orderTypeLise:[
					...
					{name:'待付款',icon:require('./image/l1.png'),badge:1},
					{name:'待发货',icon:require('./image/l2.png'),badge:2},
					{name:'待收货',icon:require('./image/l3.png'),badge:6},
					{name:'待评价',icon:require('./image/l4.png'),badge:9},
					{name:'退换货',icon:require('./image/l5.png'),badge:0}
				],
				severList:[
					[
						{name:'我的收藏',icon:require('./image/sever/point.png')},
						{name:'优惠券',icon:require('./image/sever/quan.png')},
						{name:'红包',icon:require('./image/sever/momey.png')},
						{name:'任务',icon:require('./image/sever/renw.png')},
					],
					[
						{name:'积分明细',icon:require('./image/sever/mingxi.png')},
						{name:'抽奖',icon:require('./image/sever/choujiang.png')},
						{name:'收货地址',icon:require('./image/sever/addr.png')},
						{name:'银行卡',icon:require('./image/sever/bank.png')},
						{name:'安全中心',icon:require('./image/sever/security.png')},
						{name:'在线客服',icon:require('./image/sever/kefu.png')}
					]
				],
			};
		},
